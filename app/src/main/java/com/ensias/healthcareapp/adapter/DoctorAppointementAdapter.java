    ...
    protected void onBindViewHolder(@NonNull MyDoctorAppointementHolder myDoctorAppointementHolder, int position, @NonNull final ApointementInformation apointementInformation) {
        ...
        myDoctorAppointementHolder.approveBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                FirebaseFirestore.getInstance().collection("Client").document(apointementInformation.getPatientId()).collection("calendar")
                ...
                FirebaseFirestore.getInstance().collection("Lawyer").document(apointementInformation.getDoctorId()).collection("calendar")
                        .document(apointementInformation.getTime().replace("/","_")).set(apointementInformation);

//////////// here add patient friend to doctor

                FirebaseFirestore.getInstance().document("Client/"+apointementInformation.getPatientId()).get()
                        .addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                            @Override
                            public void onSuccess(DocumentSnapshot documentSnapshot) {
                                FirebaseFirestore.getInstance().collection("Lawyer").document(apointementInformation.getDoctorId()+"")
                                        .collection("MyClients").document(apointementInformation.getPatientId()).set(documentSnapshot.toObject(Patient.class));
                            }
                        });
                FirebaseFirestore.getInstance().document("Lawyer/"+apointementInformation.getDoctorId()).get()
                        .addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                            @Override
                            public void onSuccess(DocumentSnapshot documentSnapshot) {
                                FirebaseFirestore.getInstance().collection("Client").document(apointementInformation.getPatientId()+"")
                                        .collection("MyLawyers").document(apointementInformation.getPatientId()).set(documentSnapshot.toObject(Doctor.class));
                            }
                        });
        ...
        pathReference = FirebaseStorage.getInstance().getReference().child("LawyerProfile/"+ imageId); //storage the image
